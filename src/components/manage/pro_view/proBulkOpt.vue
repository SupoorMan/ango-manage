...
<template>
	...
	<view class="content">
		...
		<scroll-view class="goods-list"  :scroll-y="true" :scroll-with-animation="true">
			<view class="selected-all" @tap="toggleCheckAll">
				<uni-icons type="checkbox-filled" size="24" color="#f94170" v-if="selectedId.length!==0 && selectedId.length==showProds.length"></uni-icons>
				<uni-icons type="circle" size="24" color="#b5b5b5" v-else></uni-icons>
				&nbsp;&nbsp;全选
			</view>
			<view v-for="pro in showProds" :key="pro.id" class="good-card"  @tap="toggleCheck(pro)">
				<view class="">
		...
		<BulkBar @showCount="showCount" @updateProds="updateProds" @deleteProds="deleteProds"/>
		<uni-popup ref="popup" >
...
<script>
	...
	import {getProds,UpdateBatch,deletePro} from '@/service/api/pro.js'
	...
	export default {
		...
		methods:{
			toggleCheckAll(){
				if(this.selectedId.length<this.showProds.length) {
					this.selectedId = this.showProds.map(m=>m.id)
				}else{
					this.selectedId=[]
				}
			},
			...
			popOpenChange(item){
				...
				if(item){
					this.updateProds(item)
				}
				this.$refs.popup.close()
			},
			checkSelect(){ // 校验是否选中了商品
			...
			updateProds(changeKeyValue){
				if(this.checkSelect()){
					uni.showLoading()
					UpdateBatch(this.selectedId.map(m=>({id:m,...changeKeyValue}))).then(res=>{
						if(res.code===200) {
							uni.showToast({
								icon:'none',
								title:'操作成功'
