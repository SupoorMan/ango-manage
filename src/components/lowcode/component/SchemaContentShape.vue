  ...
  <div class="shape" @click.stop="setcurComponent(data)" ref="shape">
    ...
    <div v-if="show" class="shape-tab" :style="{ right: getRightStyle() }">
      <template v-if="isCurComponent(data.id)">
        <i class="iconfont icon-shanchu tab-icon f16"
          @click.stop="delComponent(content.model.componentList, data.id)"></i>
      </template>
...
const getRightStyle = () => {
  let width:any = window.getComputedStyle(shape.value).width.split('px')
...
const delComponent = (list:any, id:any) => {
